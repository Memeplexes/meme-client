import { listContests, publishContest } from "../../../api/contests.js";
...
function formatContestStatus(value) {
...
function normalizeContest(contest, index) {
  ...
  return {
    ...
    creator: contest?.creator
      ...
      ?? contest?.author
      ?? null,
    status: String(contest?.status ?? "").trim().toLowerCase() || "live",
...
class FeedContestInfo extends HTMLElement {
  constructor() {
    ...
    this.handleGoLiveButtonClick = this.handleGoLiveButtonClick.bind(this);
  }
  ...
  async handleGoLiveButtonClick() {
    if (!this.contest || this.isPublishing || this.contest.status !== "draft") {
      return;
    }
    ...
    try {
      this.contest = normalizeContest(await publishContest(this.contest.id)) || {
        ...this.contest,
        status: "live"
      };
      this.isPublishing = false;
      ...
      console.error("[feed-contest-info] failed to publish contest", error);
      this.isPublishing = false;
      this.renderContest();
    }
  }
  ...
  renderContest() {
    ...
    const canGoLive = canEdit && this.contest.status === "draft";
    this.hidden = false;
    this.innerHTML = `
      <section class="feed-contest-banner" aria-label="Current contest">
        <div class="feed-contest-copy">
          ...
          <div class="feed-contest-actions">
            ...
            ${canGoLive ? `<button type="button" class="feed-contest-edit-button" data-go-live-button ${this.isPublishing ? "disabled" : ""}>${this.isPublishing ? "Going live..." : "Go Live"}</button>` : ""}
        ...
        <div class="feed-contest-stats" aria-label="Contest metadata">
          ...
          <span class="feed-contest-stat">Status ${escapeHtml(formatContestStatus(this.contest.status))}</span>
    ...
    this.querySelector("[data-go-live-button]")?.addEventListener("click", this.handleGoLiveButtonClick);
