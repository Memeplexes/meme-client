...
class CreatorCard extends HTMLElement {
  ...
  setup(creator) {
    ...
    const onClick = event => {
      ...
      // redirect the page to /hot?c=creatorName or something like that, or dispatch an event that the page can listen to and apply the filter without a full page reload
      document.location.href = `/hot?c=${encodeURIComponent(name)}`;
      /* no longer dispatching event, won't navigate away fro the current page, but will update the feed content based on the url change and the shared filter state in the SPA router
