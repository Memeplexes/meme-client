...
//      borderRight: "1px solid var(--theme-border-medium)"
    });
    ...
    Object.assign(this.downvoteButton.style, voteButtonStyle, {
      borderLeft: "1px solid var(--theme-border-medium)"
    });
  ...
  #setHoverState(button, hovering) {
    if (hovering) {
      button.style.background =
        button === this.upvoteButton
          ? "var(--theme-accent-surface)"
          : "var(--theme-warning-surface)";
      button.style.color = button === this.upvoteButton ? "var(--theme-accent)" : "var(--theme-warning)";
      return;
    }
    this.#syncVoteState();
  }
  ...
  #syncVoteState() {
    ...
    Object.assign(this.upvoteButton.style, {
      background: userVote === 1 ? "var(--theme-info-surface)" : "transparent",
      color: userVote === 1 ? "var(--theme-info)" : "var(--theme-text-primary)"
    });
    Object.assign(this.downvoteButton.style, {
      background: userVote === -1 ? "var(--theme-warning-surface)" : "transparent",
      color: userVote === -1 ? "var(--theme-warning-soft)" : "var(--theme-text-primary)"
    });
  }
