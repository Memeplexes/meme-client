...
const MEME_MODAL_MOBILE_VIEWPORT_FIX = `
...
class MemeCard extends HTMLElement {
  ...
  static #modalViewportFixInjected = false;
  ...
  #initialize() {
    ...
    if (!MemeCard.#modalViewportFixInjected) {
      const style = document.createElement("style");
      style.textContent = MEME_MODAL_MOBILE_VIEWPORT_FIX;
      document.head.appendChild(style);
      MemeCard.#modalViewportFixInjected = true;
    }
    ...
    const openMemeModal = () => {
      ...
      if (!modal) {
        modal = customElements.get("meme-modal").ensure();
      }

      if (document.body && (modal.parentElement !== document.body || document.body.lastElementChild !== modal)) {
