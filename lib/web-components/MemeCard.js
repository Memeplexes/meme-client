...
class MemeCard extends HTMLElement {
  ...
  #initialize() {
    ...
    const authorEl = document.createElement("creator-card").setup({
      name: author,
      username: metadata.creator_username ?? metadata.username ?? metadata.handle,
    ...
    console.log('Initial vote count:', metadata);
    ...
    const voteCountLabel = document.createElement("span");
    voteCountLabel.textContent = String(voteCount);
    Object.assign(voteCountLabel.style, {
      minWidth: "2ch",
      color: "rgba(255, 255, 255, 0.88)",
      fontSize: "14px",
      fontWeight: "700",
    ...
    const onDownvote = () => {
      const state = viewState.get(this);
      if (state && voteForMeme(state, -1)) {
        state.votes -= 1;
        voteCountLabel.textContent = String(state.votes);
      }
    };
    ...
    downvoteButton.addEventListener("click", onDownvote);
    upvoteButton.addEventListener("click", onUpvote);
    this._cleanup.push(() => downvoteButton.removeEventListener("click", onDownvote));
    this._cleanup.push(() => upvoteButton.removeEventListener("click", onUpvote));
    ...
    // TODO: clicking on comments should open a panel with the comments and details about the meme, including the title, author, upload date, and any other relevant information. We can also include a form to submit new comments in this panel, create a new web component for the comment panel and render it here when the comments button is clicked.
    ...
    commentsButton.addEventListener("click", onCommentsClick);
    document.addEventListener("click", onDocumentClick);
    this.addEventListener("mouseenter", onMouseEnter);
    this.addEventListener("mouseleave", onMouseLeave);
    this.addEventListener("click", onCardClick);
    this._cleanup.push(() => commentsButton.removeEventListener("click", onCommentsClick));
    ...
    voteWrap.appendChild(voteCountLabel);
