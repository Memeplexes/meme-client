...
class UploadPanel extends HTMLElement {
  ...
  async handleUpload() {
    ...
    try {
      ...
      const uploadedUrls = await uploads.uploadFiles(safeFiles, ({ filePath = "", total = 0, uploaded = 0 }) => {
        ...
        // TODO: after upload is complete we need to calculate some metadata and then perform an api.putMeme() call in order to create database entry for file
      });
