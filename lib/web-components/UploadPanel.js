    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .dropzone {
          ...
          padding: 26px;
          border-radius: 20px;
          border: 1px dashed var(--theme-accent-border-dashed);
          background: var(--theme-accent-surface-soft);
          transition: border-color 120ms ease, background 120ms ease, transform 120ms ease;
        }

        .dropzone.active {
          border-color: var(--theme-accent-fill-solid);
        ...
        .progress-bar {
          ...
          background: linear-gradient(90deg, var(--theme-accent), var(--theme-accent-bright));
      ...
      const uploadedUrls = await uploads.uploadFiles(safeFiles, async ({ file, filePath = "", total = 0, uploaded = 0 }) => {
        ...
        if (completedCurrentFile && file && filePath) {
          ...
          await updateMeme({
            ...
            console.error(`Failed to update meme metadata for ${filePath} after upload:`, err);
