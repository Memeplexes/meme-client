...
class UploadPanel extends HTMLElement {
  ...
  render() {
    ...
    const fileName = this.selectedFile ? this.selectedFile.name : "Drop an image here or browse from your device.";
    const fileMeta = this.selectedFile
      ? `${Math.round(this.selectedFile.size / 1024)} KB`
      : "PNG, JPG, GIF, WEBP";
  ...
  setSelectedFile(file) {
    this.selectedFile = file && file.type.startsWith("image/") ? file : null;
    this.progressValue = 0;
    this.statusText = this.selectedFile ? "File selected" : "Please choose an image file.";
    this.render();
  }
  ...
  async handleUpload() {
    ...
    try {
      ...
      files[0] = createUrlSafeFile(this.selectedFile);

      const uploadedUrls = await uploads.uploadFiles(files, ({ total = 0, uploaded = 0 }) => {
        const fileProgress = total > 0 ? uploaded / total : 1;
        const completedProgress = uploadedCount / files.length;
        const combinedProgress = ((completedProgress + fileProgress / files.length) * 100);
