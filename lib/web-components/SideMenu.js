import { getTopCreators, getTrendingTags } from "../api.js";
...
class SideMenu extends HTMLElement {
  ...
  #bindSlottedItemEvents() {
    this.#getMenuItems().forEach(item => {
      ...
      item.addEventListener("click", event => {
        const nextSort = (item.dataset.navSort || "").trim();
        if (nextSort) {
  ...
  #syncLocationState() {
    const activeSort = new URLSearchParams(window.location.search).get("s") || "top";
    const menuItems = this.#getMenuItems();
    let hasActiveItem = false;

    menuItems.forEach(item => {
      const isActive = (item.dataset.navSort || "").trim() === activeSort;
