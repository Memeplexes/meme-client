...
class SearchBarTags extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (this.#initialQuery) {
      this.#inputValue = this.#initialQuery;
      this.#words = this.#parseWords(this.#initialQuery);
      this.#updateDisplay();
  ...
  set value(newValue) {
    ...
    this.#words = this.#parseWords(this.#inputValue);
    this.#updateDisplay();
    this.#emitChange();
  }
  ...
  #updateDisplay() {
    ...
    // we also need to update the url search params for consistency with the input field, which is what the meme feed listens to for changes
