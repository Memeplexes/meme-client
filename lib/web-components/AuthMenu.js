...
class AuthMenu extends HTMLElement {
  ...
  render() {
      ...
      ? `
        <div class="auth-menu">
          ...
          <div class="auth-dropdown" id="auth-dropdown" hidden>
            ...
            <!-- <button class="auth-dropdown-button" id="my-likes-button" type="button">My Likes</button> -->
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .auth-dropdown {
          ...
          background: var(--theme-overlay-98);
    ...
    const myLikesButton = this.shadowRoot.querySelector("#my-likes-button");
    ...
    if (myLikesButton) {
      myLikesButton.addEventListener("click", () => {
        const storedUsername = localStorage.getItem("memeplexes-username");
        if (storedUsername) {
          window.location.href = `?c=${encodeURIComponent(storedUsername)}&filter=liked`;
        }
      });
    } 
