...
class MemeCardSizeSlider extends HTMLElement {
  connectedCallback() {
    ...
    shadow.innerHTML = `
      <style>
        ...
        .feed-size-controls {
          ...
          /*
          background: var(--feed-control-background, var(--theme-surface-ink-88));
          border-radius: 16px;
          border: 1px solid var(--feed-control-border, var(--theme-border-soft));
          */
        }
        ...
        label {
          font-size: 0.95rem;
          font-weight: 600;
          color: var(--feed-accent-color, var(--accent-color, var(--theme-accent-strong)));
        }

        input[type="range"] {
          flex: 1 1 220px;
          accent-color: var(--feed-accent-color, var(--accent-color, var(--theme-accent-strong)));
        }
    ...
    const inputId = this.dataset.inputId || `meme-card-size-slider-${new Date().getTime()}`;
