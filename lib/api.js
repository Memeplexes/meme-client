...
// TODO: refactor to create a unified apiRequest function that handles all fetch calls, response parsing, and error handling in a consistent way. The new apiRequest() method should always include authorization headers if an access token is available
...
export async function updateAccountAlias(alias) {
  ...
  try {
    ...
    const data = await parseJsonResponse(response, "updateAccountAlias");

    logApiResponse("updateAccountAlias", data);
    ...
    logApiError("updateAccountAlias", error, { url, payload });
    throw error;
  }
}
