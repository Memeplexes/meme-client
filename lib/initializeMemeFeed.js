import { fetchMemesByCreator } from "./api.js";
...
export function initializeMemeFeed({
  ...
  function updateSearchParams({ query = "", creator = "" }) {
    ...
    if (creator) {
      url.searchParams.set("c", creator);
    } else {
  ...
  async function applyFilter(query) {
    ...
    // Remark: This should not always call searchMemes?
    
    filteredFiles = searchMemes
      ? normalizeFiles(await searchMemes({ query: trimmedQuery, limit: 100, offset: 0 }))
      : filterFiles(allFiles, trimmedQuery);
    
    if (requestId !== activeSearchRequest) {
      return;
    }

    updateSearchParams({ query: trimmedQuery });
    resetFeed();
    renderBatch();
  }

  async function applyCreatorFilter(creator) {
    const requestId = ++activeSearchRequest;
    const trimmedCreator = creator.trim();

    filteredFiles = trimmedCreator
      ? normalizeFiles(await fetchMemesByCreator(trimmedCreator))
      : allFiles;
    ...
    updateSearchParams({ creator: trimmedCreator });
    search.value = trimmedCreator;
    resetFeed();
    renderBatch();
  }
