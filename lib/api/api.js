...
export async function getMemeByFilename(filename) {
  ...
  const url = new URL(`/api/meme?filename=${encodeURIComponent(filename)}`, API_URL).toString();
...
function clearStoredSession() {
  ...
  localStorage.removeItem("memeplexes-avatar-url");
}

function handleAuthFailure(action, authError) {
  const message = authError || "Session expired";
  // alert(`Authentication error: ${message}. Please log in again.`);
  console.warn(`[api] ${action} response indicates auth error:`, message);
  clearStoredSession();
...
async function readJsonResponse(response) {
  ...
  try {
    ...
    console.warn("[api] failed to parse JSON error response", error);
    return null;
  }
}
...
export async function apiRequest(url, action, options = {}) {
  ...
  try {
    ...
    // Remark: we are setting the x-auth-error header on the server to get better,
    // granularity on auth errors, however we can't seem to access the header here in client?
    // The network debug shows the response header exists, but some issue is causing no access
    // we have tried multiple CORs settings, issue could be due to local dev env
    //console.log('x-auth-error', response.headers.get('x-auth-error'))
    ...
    if (authError || ((response.status === 401 || response.status === 403) && !responseError)) {
      handleAuthFailure(action, authError);
    }

    if (!response.ok) {
      throw new Error(responseError || `${action} request failed with status ${response.status}`);
    }
...
export function searchMemes({
  ...
  console.log('searchMemes', { query, creator, filter, dateRange, contestId, limit, offset });
  ...
  if (dateRange) {
    params.set("d", dateRange);
  }

  if (contestId) {
    params.set("contestId", contestId);
  }
  console.log('searchMemes params', params.toString());
...
export function getTrendingTags({ limit = 50, window = "30d" } = {}) {
  return fetchTags(
